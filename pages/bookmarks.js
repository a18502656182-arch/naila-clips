...
export default function BookmarksPage() {
  ...
  const [items, setItems] = useState([]); // items = [{ bookmark_id, clip_id, bookmarked_at, clip }]
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((x) => {
      const title = String(x?.clip?.title || "").toLowerCase();
      ...
      {me.logged_in ? (
        ...
        >
          {loading ? (
            ...
            filtered.map((x) => {
              const c = x.clip; // ✅ 关键：从 x.clip 取字段
              return (
                <div
                  key={x.clip_id}
                  style={{
                ...
                >
                  ...
                  <div style={{ fontSize: 12, opacity: 0.75, marginTop: 6 }}>
                    {c?.access_tier || "-"} · {c?.duration_sec ? `${c.duration_sec}s` : ""}
                  </div>
