      ...
      {isDictation ? (
        ...
        <div style={{ marginTop: 8, lineHeight: 1.55 }}>
          {(subMode === "bilingual" || subMode === "en") && (
            <div style={{ fontSize: compact ? 15 : 17, fontWeight: 700 }}>{renderEn ? renderEn(seg.en_display || seg.en || "", seg.en || "", { onClickTerm, cloze: clozeMode, clozeRevealed }) : (seg.en_display || seg.en || "-")}</div>
          )}
          {(subMode === "bilingual" || subMode === "zh") && (
            <div style={{ marginTop: subMode === "bilingual" ? 6 : 0, fontSize: compact ? 14 : 15, color: THEME.colors.muted, fontWeight: 700 }}>{seg.zh || "（暂无中文）"}</div>
          )}
        </div>
      )}
    </div>
...
export default function ClipDetailClient({ clipId, initialItem, initialMe, initialDetails, initialBookmarked, initialIsMobile = false }) {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (!initialItem) {
        ...
        try {
          ...
          if (gotItem?.title) document.title = `${gotItem.title} - 奶酪包英语场景库`;
    ...
    run();
    return () => { mounted = false; document.title = "奶酪包英语场景库"; };
  }, [clipId]);
